(function () {
  ...
  function renderAll() {
    ...
    $("foot").innerHTML = `
      ${methodBlock}
      <p class="foot-line">实时模式：<code>python backend/server.py --auto-refresh 30 --fetch-on-start</code></p>
      <p class="foot-line">手动模式：编辑 <code>js/data.js</code> → 保存 → 刷新浏览器</p>
      <p class="foot-line">© ${new Date().getFullYear()} · 数据由使用者自行核对，本页不构成投资建议。</p>
    `;
  }

  // Expose for live.js to trigger re-render
